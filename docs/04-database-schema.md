...
- 所有分數計算結果附帶 `scoring_version`，支援全量重算
...
CREATE TABLE event_dimensions (
    ...
    source           VARCHAR(20)  DEFAULT 'rule'   -- 'rule' | 'ai' | 'hybrid'
...
CREATE TABLE event_scores (
    ...
    final_score       NUMERIC(8,4) NOT NULL,    -- 最終事件分數（未正規化）
    computed_at       TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
...
CREATE TABLE country_tension_daily (
    ...
    computed_at      TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_ctd UNIQUE (country_code, date)
);
...
CREATE TABLE region_tension_daily (
    ...
    social_score      NUMERIC(5,2) NOT NULL DEFAULT 0,
    cyber_score       NUMERIC(5,2) NOT NULL DEFAULT 0,
    top_country_codes TEXT[]       NOT NULL DEFAULT '{}',   -- 前 3 高張力國家
    event_count       INTEGER      NOT NULL DEFAULT 0,
    scoring_version   VARCHAR(20)  NOT NULL DEFAULT 'v1',
    CONSTRAINT uq_rtd UNIQUE (region_code, date)
);
...
CREATE TABLE ingest_errors (
    id           BIGSERIAL    PRIMARY KEY,
    source_type  VARCHAR(50)  NOT NULL,
    error_type   VARCHAR(100) NOT NULL,   -- 'timeout' | 'http_4xx' | 'schema_mismatch' | ...
    error_detail TEXT,
    raw_data     JSONB,
...
```
raw_events (1) ──→ events (1) ──→ event_countries (N)
                            ──→ event_dimensions (1)
                            ──→ event_ai_analysis (1)
                            ──→ event_scores (N，各版本一筆)
                            ──→ news_sources (N)

country_tension_daily  ←── 聚合自 events + event_scores
region_tension_daily   ←── 聚合自 country_tension_daily
global_tension_daily   ←── 聚合自 region_tension_daily + country_tension_daily
```
...
```sql
SELECT e.event_id, e.title, e.event_type, e.risk_or_relief,
       es.base_severity, es.geo_sensitivity, es.time_decay, es.final_score,
       ea.summary_zh, ea.explanation,
       array_agg(ec.country_code ORDER BY ec.role) AS countries
FROM events e
